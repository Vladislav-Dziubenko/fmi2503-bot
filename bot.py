...
async def tips(update, context):
    """
    /tips команда - выводит умные советы по подготовке к экзаменам.
    Анализирует расписание и даёт рекомендации по предметам.
    """
    if await _reply_cooldown(update): return
    try:
        from schedule import refresh_on_request, cached_data
        from study_tips import StudyCoach
        
        # Обновляем кэш
        refresh_on_request()
        
        # Берём предстоящие события
        analysis = cached_data.get("analysis", {})
        upcoming = analysis.get("upcoming", [])
        items = cached_data.get("items", [])
        
        # Создаём анализ через StudyCoach
        coach = StudyCoach()
        study_analysis = coach.analyze_schedule(items, upcoming)
        
        # Форматируем результат
        report = coach.format_study_tips(study_analysis)
        footer = f"\n\n🕒 Данные обновлены: {cached_data.get('last_upd', '—')}"
        
        await _reply_html(update, f"{report}{footer}")
    except Exception as exc:
        logger.exception("Ошибка в команде /tips: %s", exc)
        await _reply_text(update, "❌ Ошибка при анализе расписания. Попробуй позже.")
...
def main():
    ...
    for cmd, fn in [("start", start), ("raspisanie", raspisanie), ("orar", orar),
                    ("smart", smart), ("ai", smart), ("tips", tips), 
                    ("status", status), ("clear", clear_chat)]:
